#|---------------------------------------------------------|
#|       TVRage Script by Gecko321 - P2P-Network           |
#|                        JeepXJ92@gmail.com               |
#|---------------------------------------------------------|
...
bind pub - !now tv:today
bind pub - !demain tv:tomorrow
...
set networks "A&E|ABC|ABC Family|Adult Swim|AMC|Animal Planet|BBC America|Bounce|Bravo|Cartoon Network|CBS|CineMax|CMT|Comedy Central|DIRECTV|Discovery Channel|DIY Network|E!|Food Network|FOX|Fuel TV|Fuse|FX|G4|GAC|Golf Channel|H2 TV|HBO|HDNet|HGTV|History Channel|HLN|HUB|ID: Investigation Discovery|IFC|Investigation Discovery|Lifetime|Logo|MTV|MTV2|NBC|Nick @ Nite|NickToons|Ovation TV|OWN|Oxygen|Pay-Per-View|PBS|Playboy TV|ReelzChannel|Science Channel|Showtime|Smithsonian Channel|Speed Channel|Spike TV|Style|Sundance|Syfy|Syndicated|TBS|TeenNick|The CW|TLC|TNT|Travel Channel|truTV|TV Guide Channel|TV Land|TV One|USA|Velocity|VH1"
...
proc tv:search {nick host hand chan arg} {
	...
	if {$arg == ""} {
		putserv "PRIVMSG $chan :Commandes pour le script TVRage."
		putserv "PRIVMSG $chan :-!tv <serie>"
		putserv "PRIVMSG $chan :-!last/!next <serie>"
		putserv "PRIVMSG $chan :-!now/!demain"
		putserv "PRIVMSG $chan :-!sum <serie> <SxE> Ex. !sum big bang theory 6x15"
	} else {
		...
		} elseif {$page == 0 || $page == ""} { putquick "PRIVMSG $chan :TVRage non accessible."
		} else {
	    regexp {Show Name@([A-Za-z 0-9\&\':]+)} $page gotname show_name
			if {![info exists show_name]} { putquick "PRIVMSG $chan :Pas de série !"
			}  else {    
		...
		#		putquick "PRIVMSG $chan :\00304(TVRage) \00307$show_name\017 | le $show_airtime sur $show_network | ($genre) | Serie $show_status"
		#		putquick "PRIVMSG $chan :\00304(TVRage) \00307Dernier Episode:\017 $latest_ep_date $latest_ep $latest_ep_title"
				if { [info exists show_rfc] } {    
					...
					putquick "PRIVMSG $chan :\00307(TVRage) \00314$show_name |\00314 \00307Dernier épisode:\00307 \00314$latest_ep_title ($latest_ep)($latest_ep_date) |\00314 \00307Prochain épisode:\00307 \00314$next_ep_title ($next_ep)($next_ep_date) $airing |\00314 \00307Genre:\00307 \00314$genre |\00314 \00307Status:\00307 \00314$show_status |\00314 \00307Network:\00307 \00314$show_network |\00314 \00307URL\00307 \00314http://www.tvrage.com/$show_url\00314"
		#			putquick "PRIVMSG $chan :\00304(TVRage) \00307Prochain Episode:\017 $next_ep_date $next_ep $next_ep_title $airing"
		#			putquick "PRIVMSG $chan :\00304(TVRage) \00307URL:\017 http://www.tvrage.com/$show_url"
				}
			}
		}
	}
    ...
    if {[info exists show_day]} {  
      ...
      while {[regexp {\[TIME\](.*?)\[\/TIME\]} $show_day gottime time]} {
        ...
        if {[info exists shows]} {
          ...
          if {$time_till < 600 && $time_till > 0} { putserv "PRIVMSG $chan :Les séries qui suivent vont être diffusés dans [duration $time_till]"	  
						foreach line [split $shows "\n"] {
              if {[regexp {\[SHOW\](.*?)\[/SHOW\]} $line gotshow show]} { regexp {(.*?)\^(.*?)\^(.*?)\^} $show t network show episode	    
								if {[info exists network]} { if {[regexp $network $networks]} { putserv "PRIVMSG $chan :-\00307($network)\017 $show - $episode" } ; unset network show episode }
				      }
						}
...
proc tv:tomorrow {nick host hand chan arg} {
  ...
  if {$page == "timeout"} { putserv "PRIVMSG $chan :TVRage has timed out. Please try again in a few minutes"
	} elseif {$page == 0 || $page == ""} { putserv "PRIVMSG $chan :TVRage non accessible."
  } else {
      ...
      if {[info exists show_time]} {
        ...
        foreach line [split $shows "\n"] {
          if {[regexp {\[SHOW\](.*?)\[/SHOW\]} $line gotshow show]} {
            ...
            if {[info exists network] && [regexp $network $networks]} { append new_shows "--\00307($network)\017 $show - \00314$episode\017 \n" ; unset network show episode }
					}
        }
    ...
    if {$time_offset != 0} { set show_airtime [tv:time  $next_ep_date $time] }
		if {[info exists show_rfc]} {
			if {$tv_time == "on"} {
				...
				set airing "- [duration [expr {$show_date_time - $sec}]] à partir de maintenant"
			} else { set airing "" }
			putquick "PRIVMSG $chan :\[ $show_name \] Episode $next_ep se nommant $next_ep_title sera diffusé le $next_ep_date à $show_airtime sur $show_network $airing"
    } elseif {![info exists show_name]} { putquick "PRIVMSG $chan :Pas de série !"
    } else { putquick "PRIVMSG $chan :Le prochain épisode de $show_name n'est pas encore programmé." }
  }
}
proc tv:last {nick host hand chan arg} {
  ...
  if {$page == "timeout"} { putserv "PRIVMSG $chan :TVRage has timed out. Please try again in a few minutes"
    ...
    if {![info exists show_name]} { putquick "PRIVMSG $chan :Pas de série !"
			...
			putquick "PRIVMSG $chan :\[ $show_name \] Episode $latest_ep se nommant $latest_ep_title a été diffusé le $latest_ep_date le $show_airtime sur $show_network"
	  }
	}
}
proc tv:summary {nick host hand chan arg} {
  ...
  if {$page == "timeout"} { putserv "PRIVMSG $chan :TVRage has timed out. Please try again in a few minutes"
    ...
    regexp {Show Name@([A-Za-z 0-9\&\':]+)} $page gotname show_name
		regexp {Show URL@http://www.tvrage.com/([A-Za-z_0-9/-]+)} $page goturl show_url
		if {![info exists show_name]} { putserv "PRIVMSG $chan :Pas de série !"
    } else {
		  ...
		  regsub {<b>Source: <\/b>.*?<br>} $summary "" summary
			...
			if {![info exists summary]} { putquick "PRIVMSG $chan :Il n'y a pas de résume pour la série $show_name $ep"
			} else { putquick "PRIVMSG $chan :Résumé de $show_name $ep: [string trim $summary] : http://www.tvrage.com/$url" }
		}
	}
}
...
proc tv:help {hand idx arg} {
	...
	putdcc $idx "!now/!demain"
	putdcc $idx "!sum <show> <SxE>"
