...
##  changed by OV2                                                           ##
##  17.10.2010                                                               ##
##    *reworked htmlcode parsing (fixes problems with [])                    ##
##    +new token for original title (if present)                             ##
##    +new option to replace title with original title                       ##
##                                                                           ##
##  09.10.2010                                                               ##
##    *fixed direct results without search page                              ##
##                                                                           ##
##  08.10.2010                                                               ##
##    *use /combined page to prevent new imdb layout                         ##
##    *fixed output of a few fields                                          ##
##    *fixed display of hex codes [Henning]                                  ##
##                                                                           ##
##  20.03.2010                                                               ##
##    *removed unwanted characters and links from output                     ##
##                                                                           ##
##  05.01.2010                                                               ##
...
# =============
# TITLE             = %title               |
# ORIGINAL TITLE    = %origtitle           |    BOLD           = %bold
...
set announce(IMDBIRC) "%color4%bold%title%bold%color |(%time mins)|  %color14Genre%color %genre | %color14Note%color %rating (%votes votes) %rbar| |%color3%top250%color (%awards) |%url |\n%color14Synopsis%color %plot|"
...
#set %title to %origtitle if present?
set useorigtitle 1
...
set imdb_hexsubst [list]
set imdb_intsubst [list]
for {set x 0} {$x<256} {incr x} {
    set imdb_hexsubst [linsert $imdb_hexsubst end "&#x[format %02X $x];" "[format %c $x]"]
    set imdb_intsubst [linsert $imdb_intsubst end "&#$x;" "[format %c $x]"]
}

proc htmlcodes {tempfile} {
    global imdb_hexsubst imdb_intsubst
    set mapfile [string map $imdb_hexsubst $tempfile]
    set mapfile [string map $imdb_intsubst $mapfile]
    set mapfile [string map {&nbsp; "" &amp; "&" &quot; \" &raquo; ""} $mapfile]
    return $mapfile
}
...
proc imdb_proc { nick uhost handle chan arg } {
    global cast_linelimit instance queue_size queue_time queue_enabled imdb_timeout useorigtitle barcol1 barcol2 IMDB_DEBUG pub_or_not announce random warn_msg trigger binary IMDB_ALTERNATIVE
    ...
    if {$arg == ""} {
        ...
        puthelp "$toput :IMDb info script \002v17.10.2010\002 using \002$using\002"
    ...
    if { ([regexp {<title>IMDb.*Search} $html] == 1) } {
        ...
        set newurl "$imdburl/title/$ttcode/combined"
    ...
    # if no redirect necessary (only one match in meta), then go there
        ...
        set newurl "$imdburl/${location}combined"
    ...
    close $fid
    # collect output
    set title "N/A" ; set origtitle "N/A" ; set name "N/A" ; set genre "N/A" ; set tagline "N/A"
    ...
    ## get origtitle (if present)
    if [regexp {<span class="title-extra">(.*?)<i>\(original title\)</i></span>} $html dummy origtitle] {
        set origtitle [string trim $origtitle]
        set origtitle [htmlcodes $origtitle]
        if {$useorigtitle} { set title $origtitle }
    }
    if {$IMDB_DEBUG == 1} { putlog "IMDB_DEBUG origtitle == $origtitle" }    
    ## get director
    ...
    if {[regexp {<h5>Release Date:</h5>(.*?)</div>} $html dummy reldate]} {
        ...
        set reldate [string map {"See more" "" \n "" &amp; " & "} $reldate]
    ...
    if { [regexp {<h5>Plot:</h5>(.*?)</div>} $html dummy plot] || [regexp {<h5>Plot Summary:</h5>(.*?)</div>} $html dummy plot] } {
        #set plot [string map -nocase {"See more" "" "(view trailer)" "" "full summary" "" "add synopsis" "" "full synopsis (warning! may contain spoilers)" "" "full synopsis" "" " | " ""} $plot]
        set plot [string map {"|" ""} $plot]
    ...
    if [regexp {<h5>Awards:</h5>(.*?)</div>} $html dummy awards] {
       set awards [string map {"See more" "" \n " "} $awards]
    ...
    ## get comment
    if [regexp {<h5>User Reviews:</h5>\n(.*?)\n</div>} $html dummy comment] {
       set comment [string map {"See more" "" \n " "} $comment]
    ...
    ## get runtime
    if [regexp {<h5>Runtime:</h5>.*?([\d]+).*?</div>} $html dummy runtime] {
        regsub -all {[\n\s]+} $runtime {} runtime
    }
    if {$IMDB_DEBUG == 1} { putlog "IMDB_DEBUG runtime == $runtime" }
    ## get country
    if [regexp {<h5>Country:</h5>(.*?)</div>} $html dummy country] {
    ...
    ## get language
    if [regexp {<h5>Language:</h5>(.*?)</div>} $html dummy language] {
    ...
    if [regexp {<h5>Aspect Ratio:</h5>(.*?)</div>} $html dummy aspect_ratio] {
        ...
        set aspect_ratio [string map {"See more" ""} $aspect_ratio]
        set aspect_ratio [htmlcodes $aspect_ratio]
        set aspect_ratio [string trim $aspect_ratio]
    }
    if {$IMDB_DEBUG == 1} { putlog "IMDB_DEBUG aspect_ratio == $aspect_ratio" }
    ## get soundmix
    if [regexp {<h5>Sound Mix:</h5>(.*?)</div>} $html dummy soundmix] {
    ...
    ## get certification
    if [regexp {<h5>Certification:</h5>(.*?)</div>} $html dummy cert] {
    ...
    if {[string match "*%budget*" $output] || [string match "*%screens*" $output]} {
        ...
        if { $IMDB_ALTERNATIVE == 0 } {
           ...
           if [catch {set page2 [::http::geturl [string map {"combined" "business"} $newurl] -timeout $imdb_timeout]} error ] {
              puthelp "$toput :Error retrieving URL... try again later."
              ::http::Finish $page
              return
           }
          ...
          catch { exec $binary(CURL) [string map {"combined" "business"} $newurl] } html
        }
        ## get budget
    ...
    set output [replacevar $output "%origtitle" $origtitle]
...
putlog "IMDB info version 17.10.2010 loaded"
